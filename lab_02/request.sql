...
SELECT
    ...
    sell_date,
    price,
    MIN(price) OVER (PARTITION BY EXTRACT(YEAR FROM sell_date)) AS min_price_of_year,
    MAX(price) OVER (PARTITION BY EXTRACT(YEAR FROM sell_date)) AS max_price_of_year,
    AVG(price) OVER (PARTITION BY EXTRACT(YEAR FROM sell_date)) AS avg_price_of_year
FROM
    sales
ORDER BY
...
WITH unique_sales AS (
    SELECT
        ...
        payment_method,
        ROW_NUMBER() OVER (
            PARTITION BY dealer_id, car_id, sell_date, price, warranty_period, payment_method
            ORDER BY sale_id
        ) AS rn
    FROM
        sales
